...
import SelectBoxOpctions from '@/components/SelectBoxOpctions'
...
export default function SelectBox({ className, Is, List = [], Set }) {
  ...
  return (
    ...
    >
      <div
        className={`w-full h-14 bg-secundary border border-transparent  ${isShow ? 'rounded-lg !rounded-b-none !border-[var(--border-color-select-box)]' : 'rounded-lg'} hover:border-[var(--border-color-select-box)]   flex items-center justify-between px-5`}
      >
      ...
      {isShow ? (
        isMouseAnimation ? (
          <div
            className={`absolute w-[calc(100%-2px)] left-[1px] border-b  border-[var(--border-color-select-box)]  h-12  bg-gradient-to-b from-white/0 to-[--background-secundary] flex justify-center items-center  pointer-events-none z-10 top-[297px]`}
          >
            <AnimationMouse />
          </div>
        ) : (
